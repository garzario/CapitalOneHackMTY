...
The order in the table is the order they run: the plain files first, then the two
...
2. **Whole-line `--` comments only, and no dollar quoting.** `splitSqlStatements`
   in `migrate.ts` drops whole-line comments and splits on semicolons, so a
   `$$ ... $$` plpgsql body would be cut in half on its first internal semicolon.
   This is why the append-only guard on `ledger_events` is written as two rules
   rather than a trigger, and it is asserted in `migrate.test.ts`.
...
update or a delete would rewrite history that a judge is being shown. The two rules
at the end of `0003` make that append-only in the server. They are rules and not a
trigger for reason 2 above, and the cost is recorded there: an `update` or `delete`
