...
src/
  ...
  lib/
    api.ts            typed client for every route in docs/09-api.md, plus useEvents (SSE)
                      and executeRun, which reads the POST stream fetch has to open
    sse.ts            the SSE framing, by hand, because EventSource cannot POST
  ...
  components/         AppShell, States, Primitives, Evidence, Decision, Findings,
                      Levels, Receipt, SupplierDrawer, StatusCard
