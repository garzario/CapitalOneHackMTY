...
- Few, meaningful commits. One logical commit per PR. Of the 35 merged so far, 32 went into `dev`
...
7. **Build night mode, and what it actually cost us.** Between 2026-09-12 03:30 and the morning
   ...
   35 merged PRs, zero direct pushes to `main` or `dev`, and no branch stranded overnight.
...
|---|---|---|
| CI run | Run `34688689598` on `dev`, `verify` green in 31 s | Install with `--frozen-lockfile`, advisory lint, typecheck, tests, build. Docs-only changes are skipped by `paths-ignore`, on purpose |
| Test suite | `bun test`: 982 pass, 0 fail, 55 files, no network and no key | The number a judge can reproduce on their own laptop in about a second |
| Rubric score trend | `docs/01-rubric-mapping.md#self-score` | M1, 2026-09-12 04:48 CST: **82 of 100** under the stated G/Y/R rule. M2, M3 and M4 go here as they are scored |
