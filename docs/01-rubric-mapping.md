...
| 7 | Quality and functional demo (5) | `bun run demo` drives the five beats of `docs/10-demo-script.md` headless against a freshly seeded in-memory API as seven checks, the two extra ones being the CEP parser on its own and the consortium network inside a decision offline, exits non-zero on any of them and prints the hero ids, 1,670 tests across 97 files pass with no network, no database and no key (the 111 database cases skip unless `TEST_DATABASE_URL` names one, and they share one, so they are run a workspace at a time), and each of the four integrations that leave this repository has been run against the real provider: two outbound verification calls, one image through Gemini, a Nessie write, and the 0.01 MXN verification probe on the company's Nessie mirror | `scripts/demo.ts`, `docs/10-demo-script.md#numbers-the-screen-shows`, `apps/api/src/sentryone.test.ts`, `.github/workflows/ci.yml`, `docs/14-process.md#live-integrations-verified` with conversation ids `conv_6401m2ah87gnffctr757c34b5mdg` and `conv_2301m2ah9vnee2h8d14gpf1rb3rz`, and the deployed pair a judge can open on their own phone, `https://sentryone-one.vercel.app` over `https://api.104.238.147.69.sslip.io`, smoke tested by `scripts/deploy-vultr.ts` (PR #133) | Y |
| 8 | Substantiated business model (10) | MXN 899 per company per month, anchored against two published competitor prices and one published wage floor, breaking even at one stopped invoice of MXN 23,452 of subtotal per year, and one mechanism that compounds rather than a flat subscription: every tenant that joins the consortium improves the signal for every other tenant, an accounting firm holding thirty companies joins thirty tenants in one sale, and the marginal cost of the network is credits per payment run rather than per peso moved | `docs/05-business-model.md#who-pays-and-why-that-number`, `docs/05-business-model.md#unit-economics`, `docs/05-business-model.md#gtm-in-three-steps`, `docs/adr/0006-consortium-snowflake.md#consequences`, `docs/06-regulatory-privacy.md#86-cost-per-pull` (PR #93, #164) | G |
...
| 10 | Regulatory and operational feasibility (5) | Payer-side software, not a regulated entity, with the framework map, the verified text of CFF 69-B, 69-B Bis and 49 Bis read out of the compiled statute on the day of the build, including the thirty natural days and the seal restriction that article 49 Bis puts on our own user, and the LLM boundary enforced by a test that reads the package's own source | `docs/06-regulatory-privacy.md#1-our-legal-position`, `docs/06-regulatory-privacy.md#2-framework-map-mexico`, `docs/06-regulatory-privacy.md#33-article-49-bis-the-newest-list-and-the-clock-that-points-at-our-own-user`, `packages/extract/src/boundary.test.ts` test `names nothing from the decision layer in its code` (PR #87, PR #116, #180) | G |
