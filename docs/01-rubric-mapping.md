...
| 5 | Algorithmic logic and intelligence (9) | The intelligence is pure, dependency-free, unit-tested TypeScript a judge can read at the table, and it is scored against thirty labelled cases it was not written from | `packages/core/src/`, `packages/sat/src/sweep.ts`, `packages/seed/src/holdout/`, `bun run eval`, `GET /api/v1/metrics`, `docs/07-architecture.md#the-most-important-flow` | Y |
