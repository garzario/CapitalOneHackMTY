...
| 4 | Data foundation (6) | Deterministic synthetic Mexican ledger, fixed seed, with deliberate dirt we handle in tests | `packages/seed/src/generator.ts`, `docs/08-data-model.md#synthetic-data-methodology`, `packages/sat/src/snapshot/README.md` | R |
| 5 | Algorithmic logic and intelligence (9) | The intelligence is pure, dependency-free, unit-tested TypeScript a judge can read at the table | `packages/core/src/`, `packages/sat/src/sweep.ts`, `bun test` output, `docs/07-architecture.md#the-most-important-flow` | R |
