...
| 7 | Quality and functional demo (5) | `bun run demo` drives the five beats of the demo script headless against a freshly seeded in-memory API, exits non-zero on any of them and prints the hero ids, and the tests pass with no network, no database and no key | `scripts/demo.ts`, `docs/10-demo-script.md#numbers-the-screen-shows`, `apps/api/src/ceptinela.test.ts`, `.github/workflows/ci.yml` (PR #133) | Y |
