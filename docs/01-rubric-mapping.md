...
| 4 | Data foundation (6) | One synthetic company, eight months of CFDI and payment complements plus a Nessie-shaped bank mirror that reconciles to the cent, four hard negatives applied and measured, deterministic from one seed | `packages/seed/src/ceptinela/`, `packages/seed/src/ceptinela/ceptinela.test.ts`, `docs/08-data-model.md#synthetic-data-methodology`, `packages/sat/src/snapshot/README.md` | Y |
| 5 | Algorithmic logic and intelligence (9) | The intelligence is pure, dependency-free, unit-tested TypeScript a judge can read at the table, and the payment run on screen is its output over the seeded company rather than fixture rows | `packages/core/src/`, `packages/sat/src/sweep.ts`, `apps/api/src/assess.ts`, `bun test` output, `docs/07-architecture.md#the-most-important-flow` | Y |
...
| 7 | Quality and functional demo (5) | `bun run demo` drives the five beats of the demo script headless against a freshly seeded in-memory API and exits non-zero on any of them, and `--base <url>` runs the same beats against a deployment | `scripts/demo.ts`, `docs/10-demo-script.md#numbers-the-screen-shows`, `apps/api/src/ceptinela.test.ts`, `.github/workflows/ci.yml` | Y |
...
| 10 | Regulatory and operational feasibility (5) | We are not a regulated entity, we license to or operate under one, and the framework map says which obligations attach | `docs/06-regulatory-privacy.md#1-our-legal-position`, `docs/06-regulatory-privacy.md#2-framework-map-mexico`, `docs/06-regulatory-privacy.md#621-how-the-boundary-is-implemented`, `packages/extract/src/boundary.test.ts` | R |
