...
| 7 | Quality and functional demo (5) | `bun run demo` drives the five beats of the demo script headless against a freshly seeded in-memory API plus a sixth that puts the consortium network into a decision offline, exits non-zero on any of them and prints the hero ids, 1,550 tests across 89 files pass with no network, no database and no key and 103 more run only against a local Postgres, and each of the three integrations that leave this repository has been run against the real provider: two outbound verification calls, one image through Gemini, and a Nessie write | `scripts/demo.ts`, `docs/10-demo-script.md#numbers-the-screen-shows`, `apps/api/src/sentryone.test.ts`, `.github/workflows/ci.yml`, `docs/14-process.md#live-integrations-verified` with conversation ids `conv_6401m2ah87gnffctr757c34b5mdg` and `conv_2301m2ah9vnee2h8d14gpf1rb3rz`, and the deployed pair a judge can open on their own phone, `https://sentryone-one.vercel.app` over `https://api.104.238.147.69.sslip.io`, smoke tested by `scripts/deploy-vultr.ts` (PR #133) | Y |
