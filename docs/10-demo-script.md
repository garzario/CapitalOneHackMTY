...
**The clock, measured and not wished.** The spoken core is **770 words**, counted off this file. At
150 words a minute, which is the rate to rehearse against, that is **5:08** of speech; at the 165 a
rehearsed presenter actually reads at it is 4:40, and at the 140 a tired one reads at it is 5:30. The
...
|---|---|---|---|---|---|
| 0:00 to 0:44 | **1. Las dos perdidas.** Patricio | Nothing. The laptop is to one side on the entry screen, nobody looks at it, hands off the trackpad. The printed card face down on the table | "Dos perdidas, y ninguna se deshace. Si el SAT publica a tu proveedor en la lista del articulo 69-B, las deducciones que ya tomaste se anulan hacia atras, y tienes treinta dias para responder. Desde enero el articulo 49 Bis arranca ese mismo reloj contra el comprador, y el sello digital que se restringe es el tuyo. Y una vez que sale el SPEI, es firme e irrevocable. De un subtotal rechazado se revierte el cuarenta y seis por ciento entre ISR e IVA. La exposicion la crea la publicacion, no el pago: revisar al proveedor cuando lo diste de alta no protege nada. Esto no necesita que nadie te defraude." | Nothing. It runs with the repository as it stands | Interrupted at second five: give the one product sentence, "SentryOne lee las facturas que la empresa ya tiene y, en el momento de pagar, dice cuales pagos retener, cuales verificar y cuales liberar", and then go back to the **second** loss rather than abandoning the hook. Too much noise in the arena: say the two irreversibilities, open the screen, and say the pesos over beat 2. The laptop never opens before the first sentence ends |
| 0:44 to 1:12 | **2. La corrida en pesos.** Fabricio | `#/run`. `run-2026-09-07`, 92 instructions, MXN 2,174,210.76, and the hero figure is MXN 785,289.86 that is not leaving, split 2 held and 4 to verify. Rows sorted by pesos at risk with the alert rail on the right, level per line, `datos sinteticos` watermark visible throughout | "Jueves. Lupita Elizondo es la unica persona de administracion de un taller de veintiocho empleados en Apodaca, y arriba de ella no hay tesoreria. Noventa y dos transferencias antes del corte, y todo esto es sintetico. La cifra grande no es el total: son setecientos ochenta y cinco mil pesos que no van a salir, ordenados por pesos en riesgo, con su nivel y sus hallazgos debajo. Nunca un porcentaje." | #208 for the level per line in the totals; #207 for the design system. The run payload and `confidenceOf` are merged (#222) | The local instance on the second port, same seed, same figures. If the web does not come up, `?data=mock` renders the same run with no network and we say out loud that it is the no-network mode. If #208 has not landed, the pesos and the alert rail are already on screen: say the levels out loud and open one instruction to show the findings, and do not promise a per-level count in the totals |
| 1:12 to 1:49 | **3. La foto de WhatsApp, y la propuesta.** Fabian on the keyboard | The assistant panel opens, the screenshot is dragged into the chat, the extraction appears and an instruction card follows with its level and its evidence: two digits differ from the account paid 52 times (positions 9 and 10), the no-history chip and the consortium chip. Then "por que esta en rojo", a streamed answer with read-only `tool_call` cards above it, ending in one proposal card with the exact request body and a confirm button. Nothing runs before the click | "Asi llega un pago: una foto en WhatsApp, y la arrastro al chat. El unico modelo aqui transcribe, y sus siete herramientas son de lectura: una que escriba no existe en el tipo. Ya hay instruccion, con nivel y evidencia: difiere en dos digitos de la cuenta en la que le hemos pagado cincuenta y dos veces, y no tiene un pago atras. Pregunto por que esta en rojo y termina en una propuesta, el cuerpo exacto de la peticion, con un boton. Lee y propone; ejecuta una persona, con su nombre." | #197 for the assistant API. The drawer itself is merged (#228) and runs under `?data=mock` | The QR intake page that already exists, `#/intake`: the judge scans the card, photographs the CLABE and posts it from their own phone, and the row arrives over SSE. If their phone fails, use the team's second phone and say why. With no `GEMINI_API_KEY` the server answers 422 carrying the script, so the CLABE gets typed and the controls run anyway, slower, and we say so. **Never narrate the plaza on this line:** both hero accounts are `012180...`, same bank and same plaza, so the finding cannot fire there. Say the two digits and the 52 payments, and show the plaza on a line that carries it |
| 1:49 to 2:14 | **4. El SAT publica.** Adan | `#/sat`. `Simular publicacion 69-B`, `definitivo`, on the synthetic RFC. Eight months of ledger replay under three seconds, counters climbing to base 878,592.59, ISR 263,577.78, IVA 140,574.81, exposure 404,152.59. Back on `#/run`, `INS-2026-09-07-070` has moved on its own, the held split is 3 and 3 with MXN 676,112.38 held, and the pesos at risk go from 799,209.86 to 1,203,362.45. The lookup box is left free for the judge | "Reproducimos ocho meses de bitacora en pesos: ochocientos setenta y ocho mil de base ya deducida y cuatrocientos cuatro mil de exposicion. Y mira la corrida: esa linea se volvio a decidir sola mientras estabamos en la otra pestana. Teclea tu un RFC real, la lista es la oficial: catorce mil doscientos treinta y cuatro renglones, aparte de nuestras facturas sinteticas." | The sweep, the re-score and the 49 Bis coverage are merged (#175, #185). #214 is the screen polish; #204 is what would make the line read `cancelado` | The lookup box alone, answering with no network from the committed snapshot, which is the half of this beat that is real data. If the replay stalls, the recorded video cued here. If the run has not moved, say the stream dropped and reload the tab once: the totals are stored, not streamed. **Say `Retener`, not `cancelado`, until #204 is merged and the screen says it**, because today the re-score moves that line from `Verificar` to `Retener`, signed `system` |
...
also Q&A cards, so nothing is lost by leaving them out, and the full version is 1,029 words, which is
6:52 at 150 words a minute and is no longer a pitch.

| + | Beat | Said, in Spanish | Words |
|---|---|---|---|
| 1 | 6 | "Y esto se lleva el contador, una pagina: los dos articulos, la cuenta, el comprobante, el nivel, la decision y quien la firmo." | 23 |
...
|---|---|---|---|
| 0 | Nothing. The eleven beats | | 770 w, 5:08 |
| 1 | Beat 10, the four layers. It is slide 11 and Q&A card 8 | 39 w, 16 s | 731 w, 4:52 |
| 2 | Beat 6, the run executing. Falls back to the layout export or `bun run demo` | 39 w, 16 s | 692 w, 4:37 |
| 3 | Beat 9's price half, keeping the segment and the channel | 30 w, 12 s | 662 w, 4:25 |
| 4 | Beat 5, the cent, if and only if the rail is already broken. It is the hardest beat to fake and the last demo beat to give up | 81 w, 32 s | 581 w, 3:52 |
| 5 | Beat 8, the competition, held entirely for the question | 57 w, 23 s | 524 w, 3:30 |
| 6 | Beat 4's replay half, keeping the real-RFC lookup box | 28 w, 11 s | 496 w, 3:18 |
...
- **One browser window**, the tabs in order `#/run`, the assistant open on it, `#/sat`, `#/cep`,
  `#/payments`, plus one tab on `GET /api/v1/rails`.
...
| 3:20 to 4:00 | **5. The metrics page, and the business line** | Open the metrics page. For an engineer judge, open the detector beside its test file instead and run `bun test` | Precision, recall and false-positive rate with the case count next to them, per detector, plus the note naming anything we measured and refused to ship, and the level matrix beside the per-detector one. 35 labelled holdout cases: precision 87.0, recall 83.3, false-positive rate 1.6, the labelled action chosen on 33 of 35, and 12 of 12 lines that should have read `confiable` did | "No case was edited to make a control pass, and the four that still disagree are counted against us with both arguments written down. Read the level matrix too, because that is the view the clerk gets. Nothing here is a language model: the decision is deterministic and you can read it." | `bun test` output already captured in the terminal, or the metrics JSON from `GET /api/v1/metrics` |
...
| Hero instruction | `INS-2026-09-07-047` | The CLABE two digits off (positions 9 and 10), 38,417.48 MXN, verificar. Beat 4 sends the cent on this one and the CEP names the supplier, so it ends `PAGO LIBERADO` |
...
| The labelled evaluation | 35 labelled cases, precision 87.0 percent, recall 83.3 percent, false-positive rate 1.6 percent, labelled action on 33 of 35, and `confiable` 12 of 12 | `bun run eval` and `GET /api/v1/metrics`, re-read on 2026-09-13. Re-run it before quoting it: these four move with every merge |
