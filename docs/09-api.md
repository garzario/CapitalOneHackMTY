...
| POST | `/api/v1/instructions/:id/decide` | `{ action: "hold" \| "verify" \| "release", decidedBy, reason? }` | a person confirms. A release on a line a definitive SAT listing cancelled is the one shape that needs `X-Actor` with `role=owner` and a non-empty `reason`; see "What a definitive SAT listing does" below. Appends `decision_made`, carrying `decidedBy` and `reason` on the decision, so a release nobody can explain later is not a thing this product allows. Answers `{ instruction, decision, amountAtRisk, hold }`: `amountAtRisk` is the largest single amount at risk among the findings, stated rather than left to be re-derived, and `hold` is `null` exactly when the action is `release`. `reason` is optional in the contract and asked for by the screen on an override: an API that refused a release with no prose would be refused by the clerk instead, outside the product, where nothing is recorded at all. |
...
- **It lands on the ledger.** `POST /api/v1/sat/publish` appends one `payment_cancelled` per line the
  ...
  that arrives naming a supplier already listed. `reason` names the article, the list version and the
  publication date, and `actor` is absent: nobody dropped the line by hand.
- **Once per line.** A second publication naming the same supplier re-scores the pesos and appends no
  second cancellation. The event fires on the publication that made the listing definitive.
- **Only an owner reopens it, with a written reason.** `POST /api/v1/instructions/:id/decide` with
  `action: "release"` on such a line requires an `X-Actor` carrying `role=owner` whose `name` matches
  `decidedBy`, and a non-empty `reason`. `400 bad_request` for a missing or malformed header, naming
  the header, and for a name that disagrees with the body. `403 forbidden` for a `clerk`.
  `422 unprocessable` for a release with no reason, which is the one place `reason` is required rather
  than asked for by the screen: reopening a payment whose invoices have no fiscal effect is exactly the
  decision somebody has to be able to explain in eighteen months. Nothing is deleted, the
  `payment_cancelled` stays on the ledger, and the line then reads `liberado` because
  `releasedByAPerson` makes the signature outrank the listing. ADR-0002 forbids the product overruling
  a person in either direction.
- A `hold` or a `verify` on the same line goes through unchanged and the line stays `cancelado`: only
  a release reopens it.
- **Scope note, honestly.** Making EVERY write carry `X-Actor` is issue #199. What landed with the
  levels is this one check, behind the header name and the exact grammar documented above, in
  `apps/api/src/actor.ts`. Every other write still accepts a request with no header until that issue
  lands, at which point the parser there becomes the middleware and this check reads the actor off the
  context instead.
