...
Two commands worth knowing about. `bun test` runs 982 tests across 55 files with no network, no
database and no API key, which is the fastest way to check that the intelligence is real. `bun run
demo` drives the demo path headless and must be green before any rehearsal or judge visit.
...
36 hours, four people, branch and PR on every change, 35 merged so far and zero direct pushes to
